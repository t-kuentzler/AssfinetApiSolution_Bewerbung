    ...
    public class SparteService : ISparteService
    {
        ...
        public async Task ImportSpartenDatenAsync(List<object> spartenModels)
        {
            ...
            foreach (var sparteModel in spartenModels)
            {
                ...
                {
                    _logger.LogError(
                        $"ArgumentNullException beim Importieren von den Spartendaten mit dem Key '{(sparteModel as dynamic).Key}': {ex.Message}",
                        ex);
                    throw;
                }
                ...
                {
                    _logger.LogError(
                        $"InvalidOperationException beim Importieren von den Spartendaten mit dem Key '{(sparteModel as dynamic).Key}': {ex.Message}",
                        ex);
                    throw;
                }
                ...
                {
                    _logger.LogError(
                        $"Validierungsfehler bei Spartendaten mit dem Key '{(sparteModel as dynamic).Key}': {ex.Message}",
                        ex);
                    throw;
                }
                ...
                {
                    _logger.LogError(
                        $"UnknownSparteException beim Importieren von den Spartendaten mit dem Key '{(sparteModel as dynamic).Key}': {ex.Message}",
                        ex);
                    throw;
                }
                ...
                {
                    _logger.LogError(
                        $"Repository-Fehler beim Import von Spartendaten mit dem Key '{(sparteModel as dynamic).Key}'.",
                        ex);
                    throw;
                }
                catch (Exception ex)
                {
                    _logger.LogError(
                        $"Unerwarteter Fehler beim Import von Spartendaten mit dem Key '{(sparteModel as dynamic).Key}'.",
                        ex);
